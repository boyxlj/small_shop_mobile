 ...
 <div class="car">
  <div class="content">
    <van-swipe-cell wx:if="{{carList.length}}"  bind:open="onOpen" id="swipe-cell2" right-width="{{ 89 }}" v-for="item in carOrderData" :key="item.detailId"  >
              <div class="item" >
                <div class="tops">
                  <div class="tops_left"><van-checkbox :value="item.checked" data-carId="{{item.carId}}" bind:change="onChangeCheck"></van-checkbox></div>
                  <div class="right"><van-tag size="large" round type="danger" >微商城自营店</van-tag></div>
                </div>
                <div class="center">
                  <img  bindtap="shopDetail" data-detailId="{{item.detailId}}" :src="item.titleImg" />
                  <div class="center_right">
                    <div  bindtap="shopDetail" data-detailId="{{item.detailId}}" class='title'>{{item.title}}</div>
                    <div  bindtap="shopDetail" data-detailId="{{item.detailId}}" class='descs'> {{item.descs}}</div>
                    <div class='price'>
                      <div>
                        ￥{{item.price *item.num}}
                      </div>
                      <div>
                        <van-stepper v-model="item.num" max="6" :name="item.carId"   @change="onChangeNum" />
                      </div>
                    </div>
                  </div>
                </div>
...
//切换数量
const onChangeNum = async(	num: number, detail: { name: string })=>{
  console.log(num,detail.name)
  // console.log(num,carId)
  // console.log('userId,num,carId:',userId,num,carId)
  // const {data:res}  = await updateCarNum(userId,num,carId).finally(()=>{
  //   // return
  // }) 
  // console.log(res)
  // if(res.code!==200){
...
.likes {
  ...
  .tip {
    ...
    background-img: linear-gradient(to right bottom, rgb(241, 33, 33),
        rgb(228, 52, 219),
        black,
        orange);
