...
const totalNum = computed(()=>{
  const t = carOrderData.value?.filter(item=>item.checked).reduce((total,item)=>{
    
  })
  const ts =  carOrderData.value?.reduce((t:any,item)=>{
    if(item.checked){
      t+=item.num
      return t
    }
...
const totalPrice = computed(()=>{
  // return carOrderData.value.reduce((total,item)=>{
  //   if(item.checked){
  //     total+=item.num
  //   }
  // },0)
