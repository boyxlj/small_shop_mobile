<template>
  <div class="category">
    <van-sidebar  class="sidebar" v-model="active" >
      <van-sidebar-item disabled @click="change(item.detailId)" v-for="item in categoryData" :key="item.detailId" :title="item.categoryName+''" />
    </van-sidebar>
    <div class="shop">
...
const categoryShop = async () => {
  ...
  const { data: res } = await getCategoryShop(req).finally(()=>toast.clear());
  if (res.code != 200) {
