<template>
  ...
  <div class="item" v-for="(item,index) in orderData" :key="index" >
    ...
    <div class="shopList" bindtap="tiaoDetail"  v-for="item1 in item.orderList" :key="item1.orderId" >
      <img :src="item1.titleImg" />
      <div class="right">
   <!--  <div class="title">{{item.title}}</div>-->
        <div class="title">{{item1.title}}/{{item1.descs}}</div> 
        <div class="price"><span>￥{{item1.price}}</span><span>x{{item1.prePrice}}</span></div>
      </div>
    </div>
    <div class="btns">
      <van-button  size="small" bind:click="deleteOrder(item.orderNumber)"  round>删除订单</van-button>
      <van-button @click="pay(item.orderNumber)" size="small" style="margin-left:20rpx;" v-if="item.type=='2'"  type="danger" round>立即支付</van-button>
    </div>
  </div>
