...
<van-dialog use-slot title="请扫码支付"  v-model:show="payDialogShow" show-cancel-button  @confirm="confirmPay" confirmButtonspan="我已支付" @cancel="cancelPay">
  <div style="width: 100%;height: 12.5rem; padding: 1.875rem 0; display: flex;justify-content: center;" class="pays">
    <img style="width: 12.5rem;height: 100%;" src="https://api.helloxlj.top/public/static/file165680971959065391023.png" />
  </div>
</van-dialog>
  </div>
</template>
...
const submitOrder= async()=>{
  ...
  const list:ICarOrderData[] = JSON.parse(route.query.list) as any
    const carIdList = JSON.parse(list).map(item=>item.carId)
